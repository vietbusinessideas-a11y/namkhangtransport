...
// DATA & DEFAULTS
// ═══════════════════════════════════════
var DB={hopDong:[],thuChi:[],xe:[],taiXe:[],khachHang:[],giaDau:[]};
...
function mapGiaDau(r){return{id:r.id,gia:Number(r.gia),ngay:r.ngay_ap_dung||'',ghiChu:r.ghi_chu||'',created_at:r.created_at||''};}
...
function openGiaDauModal(){
  ...
  var histRows = DB.giaDau.slice().sort(function(a,b){return b.ngay.localeCompare(a.ngay);}).map(function(g,i){
    var isCur = cur && g.id === cur.id;
    var endDate = i===0 ? '<span style="color:var(--green);font-weight:700">Đang áp dụng</span>' : '';
    return '<tr style="'+(isCur?'background:#f0fdf4':'')+'"><td>'+fmtD(g.ngay)+'</td><td><span class="mono" style="font-weight:700">'+fmt(g.gia)+'</span> đ/lít</td><td style="color:var(--text3);font-size:.75rem">'+(g.ghiChu||'—')+'</td><td>'+endDate+'</td></tr>';
  }).join('');
  var histTable = DB.giaDau.length
    ? '<div class="table-wrap"><table class="dt" style="width:100%"><thead><tr><th>Ngày áp dụng</th><th>Giá</th><th>Ghi chú</th><th>Trạng thái</th></tr></thead><tbody>'+histRows+'</tbody></table></div>'
    : '<div style="text-align:center;padding:24px;color:var(--text3);font-size:.82rem">Chưa có lịch sử giá dầu</div>';

  showModal('⛽ Quản lý giá dầu tham chiếu', curStr,
    (isAdmin() ? '<div style="background:var(--surface2);border-radius:10px;padding:14px;margin-bottom:16px">'
      +'<div style="font-size:.75rem;font-weight:700;color:var(--text2);margin-bottom:10px">➕ Cập nhật giá mới</div>'
      +'<div class="form-row"><div class="fg"><label class="fl">Giá mới <span class="req">*</span> <span style="font-size:.68rem;color:var(--text3)">(đ/lít)</span></label><input type="text" inputmode="numeric" class="fc" id="gd-gia" placeholder="VD: 22.000" oninput="fmtInput(this)"></div>'
      +'<div class="fg"><label class="fl">Ngày áp dụng <span class="req">*</span></label><input type="date" class="fc" id="gd-ngay" value="'+today+'"></div></div>'
      +'<div class="fg"><label class="fl">Ghi chú</label><input class="fc" id="gd-ghi-chu" placeholder="VD: Xăng RON 95-III tháng 4/2026"></div>'
      +'</div>' : '')
    +'<div style="font-size:.75rem;font-weight:700;color:var(--text2);margin-bottom:8px">📋 Lịch sử giá dầu</div>'
    + histTable,
    '<button class="btn btn-ghost" onclick="closeModal()">Đóng</button>'
    +(isAdmin() ? '<button class="btn btn-accent" onclick="saveGiaDau()">💾 Lưu giá mới</button>' : '')
  );
}

function saveGiaDau(){
  if(!requireAdmin()) return;
  var giaVal  = parseInt((document.getElementById('gd-gia').value||'').replace(/\D/g,''));
  var ngayVal = (document.getElementById('gd-ngay').value||'').trim();
  var ghiChu  = (document.getElementById('gd-ghi-chu').value||'').trim();
  if(!giaVal){ toast('⚠️ Vui lòng nhập giá dầu','error'); return; }
  if(!ngayVal){ toast('⚠️ Vui lòng chọn ngày áp dụng','error'); return; }
  sbPost('gia_dau',{gia:giaVal,ngay_ap_dung:ngayVal,ghi_chu:ghiChu||null})
    .then(function(res){
...
function loadXeKmStats(baseKm, bienSo, hdDoneCount){
  ...
  Promise.all([
    sbFetch('bao_cao','bien_xe=eq.'+bienEnc+'&loai=eq.km_cuoi&so_km=not.is.null&select=so_km,hd_so,created_at&order=so_km.desc&limit=1'),
    sbFetch('bao_cao','bien_xe=eq.'+bienEnc+'&loai=eq.km_dau&so_km=not.is.null&select=so_km,hd_so,created_at&order=so_km.asc&limit=1')
  ]).then(function(res){
...
function renderFuelSummary(rows, xe){
  ...
  if(calcMode === 'standard_rate'){
    ...
    coLit    = true;
    var giaDauRef = xe.giaDauTK || (getCurrentGiaDau() ? getCurrentGiaDau().gia : null);
    if(giaDauRef){
      tongTien = Math.round(tongLit * giaDauRef);
      coTien   = true;
      var giaNguon = xe.giaDauTK ? 'giá riêng xe' : ('giá TT '+fmtD(getCurrentGiaDau().ngay));
      dinhMucNote = 'Định mức '+xe.dinhMuc+' L/100km × '+fmtM(giaDauRef)+'/lít ('+giaNguon+')';
    } else {
      dinhMucNote = 'Định mức '+xe.dinhMuc+' L/100km — chưa có giá dầu tham chiếu';
    }
  }
